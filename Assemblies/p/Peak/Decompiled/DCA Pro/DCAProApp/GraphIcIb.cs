...
internal class GraphIcIb : Graph
{
  ...
  public GraphIcIb(frmDCAProApp Parent)
    : base(Parent)
  {
    this.Type = Graph.Types.GRAPH_ICIB;
    this.TestType = Test.TYPE.BJT;
    this.theMenuItem = this.theForm.IcIbBJTMenuSubItem;
    this.thisPanel = this.theForm.panelIcIb;
    this.thisZedGraph = this.theForm.zedGraphIcIb;
    this.thisStartButton = this.theForm.butStartIcIb;
    this.thisAutosetButton = this.theForm.butAutosetIcIb;
    this.thisCheckLockParameters = this.theForm.checkIcIbLockParameters;
    this.InitControls();
    this.theForm.textIcIbVcMin.Validating += new CancelEventHandler(((Graph) this).ValidateText);
    this.theForm.textIcIbVcMax.Validating += new CancelEventHandler(((Graph) this).ValidateText);
    this.theForm.textIcIbBaseuIMin.Validating += new CancelEventHandler(((Graph) this).ValidateText);
    this.theForm.textIcIbBaseuIMax.Validating += new CancelEventHandler(((Graph) this).ValidateText);
    this.theForm.textIcIbPoints.Validating += new CancelEventHandler(((Graph) this).ValidateText);
    this.theForm.textIcIbTraces.Validating += new CancelEventHandler(((Graph) this).ValidateText);
    this.SetGraphDefaults();
    this.MinYSpan = 20.0;
    this.MinYSpanRound = 5.0;
    this.GraphType = "BJT Ic / Ib";
    this.thisZedGraph.GraphPane.Title.Text = this.GraphType;
    this.thisZedGraph.GraphPane.XAxis.Title.Text = "Ib (µA)";
    this.thisZedGraph.GraphPane.YAxis.Title.Text = "Ic (mA)";
    this.XAxisUnits = "µA";
    this.YAxisUnits = "mA";
    this.theForm.textIcIbVcMin.Text = $"{(ValueType) 2f:F1}";
    this.theForm.textIcIbVcMax.Text = $"{(ValueType) 10f:F1}";
    this.theForm.textIcIbPoints.Text = $"{51:D}";
    this.theForm.textIcIbTraces.Text = $"{5:D}";
    this.theForm.textIcIbBaseuIMin.Text = $"{(ValueType) 0.0f:F1}";
    this.theForm.textIcIbBaseuIMax.Text = $"{(ValueType) 0.0f:F1}";
    this.UpdateParameterTooltips(this.ToolTipGraph);
    this.CircuitSmall = this.theForm.picBJTIcIbCircuitSmall;
    this.CircuitLarge = this.theForm.picBJTIcIbCircuitLarge;
  ...
  {
    theTooltip.SetToolTip((Control) this.theForm.textIcIbBaseuIMin, $"Applied Ib for each point.\r\n{(ValueType) 0.0f:F1}µA to {(ValueType) 10000f:F1}µA");
    theTooltip.SetToolTip((Control) this.theForm.textIcIbBaseuIMax, $"Applied Ib for each point.\r\n{(ValueType) 0.0f:F1}µA to {(ValueType) 10000f:F1}µA");
    theTooltip.SetToolTip((Control) this.theForm.textIcIbVcMin, $"Applied Vce for each trace.\r\n{(ValueType) 0.0f:F1}V to {(ValueType) 10f:F1}V");
    theTooltip.SetToolTip((Control) this.theForm.textIcIbVcMax, $"Applied Vce for each trace.\r\n{(ValueType) 0.0f:F1}V to {(ValueType) 10f:F1}V");
  }
  ...
  internal override bool SetNewDefaults(Test.unResultDevice Result)
  {
    if (Result.Type == Test.TYPE.BJT)
    {
      ...
      this.theForm.rtextIcIbConfig.RenderLeadString(Result.BJT.Config, "E", "C", "B");
      if (!this.ParametersLocked)
      {
        ...
        if (this.IMax < 0.0)
        {
          this.RIbMax = 0.0;
          this.RIbMin = 0.0;
          this.theForm.textIcIbPoints.Text = "1";
        }
        ...
        else if (this.IMax / 3.0 < 0.0)
        {
          ...
          this.theForm.textIcIbPoints.Text = "2";
        }
        else if (this.IMax / 4.0 < 0.0)
        {
          this.RIbMin = Math.Round(this.IMax / 3.0);
          this.RIbMax = this.RIbMin * 3.0;
          this.theForm.textIcIbPoints.Text = "3";
        }
        else if (this.IMax / 5.0 < 0.0)
        {
          this.RIbMin = Math.Round(this.IMax / 4.0);
          this.RIbMax = this.RIbMin * 4.0;
          this.theForm.textIcIbPoints.Text = "4";
        }
        else
        {
          this.RIbMin = Math.Round(this.IMax / 5.0);
          this.RIbMax = this.RIbMin * 5.0;
          this.theForm.textIcIbPoints.Text = "5";
        }
        this.theForm.textIcIbPoints.Text = "21";
        this.theForm.textIcIbBaseuIMax.Text = $"{this.RIbMax:F1}";
        this.theForm.textIcIbBaseuIMin.Text = $"{this.RIbMin:F1}";
      }
    }
    else
    {
      this.theForm.rtextIcIbConfig.Render("Unknown");
  ...
  internal override void buttonStart_Click(object sender, EventArgs e)
  {
    try
    {
      if (!this.theForm.bgWorkerTest.IsBusy && this.thisStartButton.Text == "Start")
      {
        ...
        this.theForm.bgWorkerTest.RunWorkerAsync((object) ACT.GRAPH_ICIB);
      }
      else
      {
  ...
  internal override void ValidateText(object sender, CancelEventArgs e)
  {
    ...
    if (!float.TryParse(textBox.Text, out result))
      textBox.Text = $"{0.0:F1}";
    if (textBox == this.theForm.textIcIbVcMin)
    {
      if ((double) result < 0.0)
        result = 0.0f;
      if ((double) result > 10.0)
        result = 10f;
      float single = Convert.ToSingle(this.theForm.textIcIbVcMax.Text);
      if ((double) result > (double) single)
        result = single;
      textBox.Text = $"{result:F1}";
    }
    else if (textBox == this.theForm.textIcIbVcMax)
    {
      if ((double) result < 0.0)
        result = 0.0f;
      if ((double) result > 10.0)
        result = 10f;
      float single = Convert.ToSingle(this.theForm.textIcIbVcMin.Text);
      if ((double) result < (double) single)
        result = single;
      textBox.Text = $"{result:F1}";
    }
    else if (textBox == this.theForm.textIcIbBaseuIMin)
    {
      if ((double) result < 0.0)
        result = 0.0f;
      if ((double) result > 10000.0)
        result = 10000f;
      float single = Convert.ToSingle(this.theForm.textIcIbBaseuIMax.Text);
      if ((double) result > (double) single)
        result = single;
      textBox.Text = $"{result:F1}";
    }
    else if (textBox == this.theForm.textIcIbBaseuIMax)
    {
      if ((double) result < 0.0)
        result = 0.0f;
      if ((double) result > 10000.0)
        result = 10000f;
      float single = Convert.ToSingle(this.theForm.textIcIbBaseuIMin.Text);
      if ((double) result < (double) single)
        result = single;
      textBox.Text = $"{result:F1}";
    }
    else if (textBox == this.theForm.textIcIbPoints)
    {
    ...
    {
      if (textBox != this.theForm.textIcIbTraces)
        return;
      if ((double) result < 1.0)
        result = 1f;
  ...
  internal override void DoGraph(BackgroundWorker worker, WorkResult Info)
  {
    try
    {
      int int16_1 = (int) Convert.ToInt16(this.theForm.textIcIbTraces.Text);
      int int16_2 = (int) Convert.ToInt16(this.theForm.textIcIbPoints.Text);
      float[] numArray1 = new float[int16_2];
      double[] numArray2 = new double[int16_2];
      double[] numArray3 = new double[int16_2];
      double num1 = Convert.ToDouble(this.theForm.textIcIbBaseuIMax.Text) / 1000.0;
      double num2 = Convert.ToDouble(this.theForm.textIcIbBaseuIMin.Text) / 1000.0;
      bool flag1 = false;
      double num3 = Convert.ToDouble(this.theForm.textIcIbVcMin.Text);
      double num4 = Convert.ToDouble(this.theForm.textIcIbVcMax.Text);
      ...
      for (int index2 = 0; index2 < int16_1; ++index2)
      {
        ...
        for (int index3 = 0; index3 < int16_2; ++index3)
        {
          ...
          if (!flag2)
          {
            ...
            numArray3[index3] = (double) this.theForm.thisDCAPro.DetermineIb(thisConfig) * 1000.0;
            int count = ((List<CurveItem>) this.thisZedGraph.GraphPane.CurveList).Count;
            if (index3 == 0)
            {
              index1 = this.TraceAddWhole(new Graph.Trace($"{this.ParseTracePrefix((Control) this.theForm.textGlobalTracePrefix, (Control) this.theForm.numGlobalTraceN)}Vce={num6:F2}V", "", this.GetNextColor(), numArray3[0], numArray2[0]), count);
            }
            else
            {
              thisCurve = ((List<CurveItem>) this.thisZedGraph.GraphPane.CurveList)[index1];
              thisCurve.AddPoint(numArray3[index3], numArray2[index3]);
            }
